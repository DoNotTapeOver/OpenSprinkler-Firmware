...
struct StationAttrib {	// station attributes
	uint8 mas:1;
	uint8 igs:1;	// ignore sensor 1
	uint8 mas2:1;
	uint8 dis:1;
	uint8 seq:1;
	uint8 igs2:1;// ignore sensor 2
	uint8 igrd:1;// ignore rain delay
	uint8 unused:1;
	
	uint8 gid:4; // group id: reserved for the future
	uint8 dummy:4;
	uint8 reserved[2]; // reserved bytes for the future
...
struct StationData {
	...
	uint8 type; // station type
	uint8 sped[STATION_SPECIAL_DATA_SIZE]; // special station data
};
...
struct HTTPStationData {
	uint8 data[STATION_SPECIAL_DATA_SIZE];
};

/** Volatile controller status bits */
struct ConStatus {
	uint8 enabled:1;						// operation enable (when set, controller operation is enabled)
	uint8 rain_delayed:1;			// rain delay bit (when set, rain delay is applied)
	uint8 sensor1:1;						// sensor1 status bit (when set, sensor1 on is detected)
	uint8 program_busy:1;			// HIGH means a program is being executed currently
	uint8 has_curr_sense:1;		// HIGH means the controller has a current sensing pin
	uint8 safe_reboot:1;				// HIGH means a safe reboot has been marked
	uint8 req_ntpsync:1;				// request ntpsync
	uint8 req_network:1;				// request check network
	uint8 display_board:5;			// the board that is being displayed onto the lcd
	uint8 network_fails:3;			// number of network fails
	uint8 mas:8;								// master station index
	uint8 mas2:8;							// master2 station index
	uint8 sensor2:1;						// sensor2 status bit (when set, sensor2 on is detected)
	uint8 sensor1_active:1;		// sensor1 active bit (when set, sensor1 is activated)
	uint8 sensor2_active:1;		// sensor2 active bit (when set, sensor2 is activated)
	uint8 req_mqtt_restart:1;			// request mqtt restart
};
...
public:
...
#if defined(OSPI)
	static uint8 pin_sr_data;		// RPi shift register data pin
															// to handle RPi rev. 1
#endif
	...
	static uint8 nboards, nstations;
	static uint8 hw_type;	// hardware type
	static uint8 hw_rev;		// hardware minor

	static uint8 iopts[]; // integer options
	static const char*sopts[]; // string options
	static uint8 station_bits[];			// station activation bits. each byte corresponds to a board (8 stations)
																	// first byte-> master controller, second byte-> ext. board 1, and so on
	// todo future: the following attribute bytes are for backward compatibility
	static uint8 attrib_mas[];
	static uint8 attrib_igs[];
	static uint8 attrib_mas2[];
	static uint8 attrib_igs2[];
	static uint8 attrib_igrd[];
	static uint8 attrib_dis[];
	static uint8 attrib_seq[];
	static uint8 attrib_spe[];
	...
	static uint8  button_timeout;				// button timeout
	...
	static uint8  weather_update_flag; 
	// member functions
	// -- setup
	...
	static uint8 start_network();	// initialize network with the given mac and port
	static uint8 start_ether();	// initialize ethernet with the given mac and port	
	static bool network_connected();		// check if the network is up
	static bool load_hardware_mac(uint8* buffer, bool wired=false);	// read hardware mac address
	static time_t now_tz();
	// -- station names and attributes
	static void get_station_data(uint8 sid, StationData* data); // get station data
	static void set_station_data(uint8 sid, StationData* data); // set station data
	static void get_station_name(uint8 sid, char buf[]); // get station name
	static void set_station_name(uint8 sid, char buf[]); // set station name
	static uint8 get_station_type(uint8 sid); // get station type
	//static StationAttrib get_station_attrib(uint8 sid); // get station attribute
	...
	static bool sopt_save(uint8 oid, const char *buf);
	static void sopt_load(uint8 oid, char *buf);
	static String sopt_load(uint8 oid);

	static uint8 password_verify(char *pw);	// verify password
	...
	static uint8 detect_programswitch_status(ulong); // get program switch status
	...
	static uint8 weekday_today();		// returns index of today's weekday (Monday is 0)

	static uint8 set_station_bit(uint8 sid, uint8 value); // set station bit of one station (sid->station index, value->0/1)
	static void switch_special_station(uint8 sid, uint8 value); // swtich special station
	...
	static void lcd_print_line_clear_pgm(PGM_P str, uint8 line);
	#else
	static void lcd_print_pgm(PGM_P PROGMEM str);						// print a program memory string
	static void lcd_print_line_clear_pgm(PGM_P PROGMEM str, uint8 line);
	#endif
	static void lcd_print_time(time_t t);									 // print current time
	static void lcd_print_ip(const uint8 *ip, uint8 endian);		// print ip
	static void lcd_print_mac(const uint8 *mac);							// print mac
	static void lcd_print_station(uint8 line, char c);				// print station bits of the board selected by display_board
	static void lcd_print_version(uint8 v);									 // print version number
	...
	// -- UI and buttons
	static uint8 button_read(uint8 waitmode); // Read button value. options for 'waitmodes' are:
	...
	static void lcd_set_brightness(uint8 value=1);
	...
	static void set_screen_led(uint8 status);	
	static uint8 get_wifi_mode() {return wifi_testmode ? WIFI_MODE_STA : iopts[IOPT_WIFI_MODE];}
	static uint8 wifi_testmode;
...
private:
	...
	static uint8 button_read_busy(uint8 pin_butt, uint8 waitmode, uint8 butt, uint8 is_holding);

	#if defined(ESP8266)
	static void latch_boost();
	static void latch_open(uint8 sid);
	static void latch_close(uint8 sid);
	static void latch_setzonepin(uint8 sid, uint8 value);
	static void latch_setallzonepins(uint8 value);
	static void latch_disable_alloutputs_v2();
	static void latch_setzoneoutput_v2(uint8 sid, uint8 A, uint8 K);
	static void latch_apply_all_station_bits();
	static uint8 prev_station_bits[];
	#endif
#endif // LCD functions
	static uint8 engage_booster;
};
