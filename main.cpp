	...
	switch(type) {
		...
		case NOTIFY_FLOWSENSOR:
			...
			if (os.mqtt.enabled()) {
				...
				sprintf_P(payload, PSTR("{\"count\":%lu,\"volume\":%d.%02d}"), lval, (long unsigned int)volume/100, (int)volume%100);
			}
			if (ifttt_enabled) {
				sprintf_P(postval+strlen(postval), PSTR("Flow count: %lu, volume: %d.%02d"), lval, (long unsigned int)volume/100, (int)volume%100);
			}
			break;
