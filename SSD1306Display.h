...
public:
	SSD1306Display(uint8_t _addr, uint8_t _sda, uint8_t _scl) : SSD1306(_addr, _sda, _scl) {
		cx = 0;
		cy = 0;
		for(uint8 i=0;i<NUM_CUSTOM_ICONS;i++) custom_chars[i]=NULL;
	}
	void begin() {
	...
	size_t write(uint8_t c) {
		...
		if(c<NUM_CUSTOM_ICONS && custom_chars[c]!=NULL) {
			drawXbm(cx, cy, fontWidth, fontHeight, (const uint8*) custom_chars[c]);
		} else {
	...
	void createChar(uint8 idx, PGM_P ptr) {
